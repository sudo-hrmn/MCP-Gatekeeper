...
async def register_upstream_mcp_server(
    ...
    """Registers an upstream MCP server with the Trust Gateway for proxy protection."""
    await init_db()
    async with AsyncSessionLocal() as db:
        s = await register_server(db, server_id, server_name, upstream_url)
...
async def get_security_incidents(limit: int = 10) -> list:
    """Returns recent flagged security incidents."""
    await init_db()
    async with AsyncSessionLocal() as db:
        res = await db.execute(select(Incident).order_by(desc(Incident.created_at)).limit(limit))
...
async def get_audit_log(limit: int = 10) -> list:
    """Returns the SHA-256 hash-chained security audit log."""
    await init_db()
    async with AsyncSessionLocal() as db:
...
@mcp.tool(
    ...
    description="Approves or denies a held high-risk tool call pending human confirmation."
)
async def resolve_human_approval(
    confirmation_id: str,
    approve: bool,
    resolved_by: str = "ai_assistant"
) -> str:
    """Approves or denies a held high-risk action."""
    await init_db()
    async with AsyncSessionLocal() as db:
...
async def dashboard_route(request):
    """Renders the Admin Dashboard UI on FastMCP Cloud."""
